...
try:
    ...
    pass
try:
    sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cutlass_evt_fusion'))
    ...
    _HAS_CUTLASS_EVT = True
except Exception:
    pass
if _HAS_TRITON_TMA:

    @triton.jit
    def _fused_leaky_relu_sq_kernel(a_desc, b_desc, c_desc, aux_desc, M, N, K, BLOCK_SIZE_M: tl.constexpr, BLOCK_SIZE_N: tl.constexpr, BLOCK_SIZE_K: tl.constexpr, GROUP_SIZE_M: tl.constexpr, NUM_SMS: tl.constexpr):
        dtype = tl.bfloat16
        ...
        for tile_id in tl.range(start_pid, num_tiles, NUM_SMS, flatten=True):
            ...
            c_desc.store([offs_am, offs_bn + BLOCK_SIZE_N // 2], c1_ag)
            c1_post = 0.5 * c1_ag * c1
            aux_desc.store([offs_am, offs_bn + BLOCK_SIZE_N // 2], c1_post)

    def _triton_fused_leaky_relu_sq(a, b):
        M, K = a.shape
        N, K2 = b.shape
        assert K == K2
        act_grad = torch.empty((M, N), device=a.device, dtype=a.dtype)
        ...
        c_desc = TensorDescriptor.from_tensor(act_grad, [BLOCK_SIZE_M, BLOCK_SIZE_N // 2])
        aux_desc = TensorDescriptor.from_tensor(post, [BLOCK_SIZE_M, BLOCK_SIZE_N // 2])

        def grid(META):
            return (min(NUM_SMS, triton.cdiv(M, BLOCK_SIZE_M) * triton.cdiv(N, BLOCK_SIZE_N)),)
        _fused_leaky_relu_sq_kernel[grid](a_desc, b_desc, c_desc, aux_desc, M, N, K, BLOCK_SIZE_M=BLOCK_SIZE_M, BLOCK_SIZE_N=BLOCK_SIZE_N, BLOCK_SIZE_K=BLOCK_SIZE_K, GROUP_SIZE_M=1, NUM_SMS=NUM_SMS, num_stages=4, num_warps=8)
        return (act_grad, post)
    ...
    class _FusedMLP(torch.autograd.Function):
        ...
        def forward(ctx, x, fc_w, proj_w):
            ...
            act_grad, post = _triton_fused_leaky_relu_sq(x_flat, fc_w)
        ...
        def backward(ctx, grad_output):
            ...
            dW_proj = go.T @ post
            if _HAS_CUTLASS_EVT:
                dpre = torch.ops.cutlass_evt.gemm_mul(go, proj_w, act_grad)
            else:
                dpre = go @ proj_w * act_grad
            dW_fc = dpre.T @ x_flat
            dx = dpre @ fc_w
            return (dx.reshape(grad_output.shape), dW_fc, dW_proj)
...
class Hyperparameters:
    ...
    mamba2_layers = os.environ.get('MAMBA2_LAYERS', '')
    ...
    hash_embed_enabled = bool(int(os.environ.get('HASH_EMBED_ENABLED', '1')))
    hash_embed_size = int(os.environ.get('HASH_EMBED_SIZE', 16384))
...
class ShuffledSequenceLoader:
    ...
    def __init__(self, h, device):
        ...
        self._window_buf = np.empty(self.seq_len + 1, dtype=np.int64)
    ...
    def next_batch(self, global_tokens, grad_accum_steps):
        ...
        for bi in range(device_batch_size):
            ...
            self._window_buf[:] = mm[start_ind:start_ind + self.seq_len + 1]
            window = torch.as_tensor(self._window_buf)
            x[bi] = window[:-1]
            y[bi] = window[1:]
...
class MLP(nn.Module):
    ...
    def forward(self, x, up_w, down_w):
        if _HAS_TRITON_TMA and x.is_cuda and self.training and self.use_fused:
...
class Mamba2Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        T_pad = T + pad
        proj = self.in_proj(x)
        x_ssm = proj[..., :H * P].reshape(B_sz, T_pad, H, P)
        z = proj[..., H * P:2 * H * P].reshape(B_sz, T_pad, H, P)
        Bm = proj[..., 2 * H * P:2 * H * P + H * N].reshape(B_sz, T_pad, H, N)
        Cm = proj[..., 2 * H * P + H * N:2 * H * P + 2 * H * N].reshape(B_sz, T_pad, H, N)
...
class _MambaAttnAdapter(nn.Module):
    """Adapter so block.attn(x, q_w, k_w, v_w, out_w) works in shared paths.
    Holds the Mamba2Layer as `.mamba`, ignores q/k/v/out_w bank tensors.
    """
...
class Mamba2Block(nn.Module):
    """Drop-in replacement for Block using Mamba2 instead of attention.
    Exposes .attn (adapter containing the mamba layer), .mlp, scales and
    resid_mix — same interface as Block. Ignores q_w/k_w/v_w/out_w passed
    in from the bank.
    """
    def __init__(self, dim, num_heads, head_dim, state_dim, mlp_mult,
                 a_log_min=-3.0, a_log_max=-1.0, use_fused=True,
...
class Block(nn.Module):
    ...
    def forward_mlp(self, x, up_w, down_w):
        return x + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(self.mlp_norm(x) * self.ln_scale_factor, up_w, down_w)
...
class GPT(nn.Module):
    ...
    def __init__(self, h):
        ...
        self.num_attn_layers = slot
        self.attn_bank_slot = attn_bank_slot
        na = max(self.num_attn_layers, 1)
        self.qo_bank = nn.Parameter(torch.empty(2 * na, h.model_dim, h.model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * na, kv_dim, h.model_dim))
        ...
        def _make_block(i):
            if i in self.mamba2_layer_set:
                return Mamba2Block(
                    h.model_dim, h.mamba2_num_heads, h.mamba2_head_dim,
        ...
        self.blocks = nn.ModuleList([_make_block(i) for i in range(h.num_layers)])
        if h.rope_dims > 0:
            head_dim = h.model_dim // h.num_heads
    ...
    def _init_weights(self):
        ...
        for i in range(na):
            ...
            nn.init.zeros_(self.qo_bank.data[na + i])
            self.qo_bank.data[na + i].mul_(proj_scale)
            nn.init.orthogonal_(self.kv_bank.data[i], gain=1.0)
            nn.init.orthogonal_(self.kv_bank.data[na + i], gain=1.0)
        for i in range(n):
    ...
    def _bank_weights(self, i):
        slot = self.attn_bank_slot[i]
        na = max(self.num_attn_layers, 1)
        if slot < 0:
            q_w = self.qo_bank[0]; k_w = self.kv_bank[0]
            v_w = self.kv_bank[na]; out_w = self.qo_bank[na]
        else:
            q_w = self.qo_bank[slot]; k_w = self.kv_bank[slot]
            v_w = self.kv_bank[na + slot]; out_w = self.qo_bank[na + slot]
        return (q_w, k_w, v_w, out_w, self.mlp_up_bank[i], self.mlp_down_bank[i])

    def _parallel_active_for_layer(self, physical_idx, virtual_idx):
    ...
    def _apply_skip_single(self, x, skip, skip_idx):
    ...
    def _apply_skip_parallel(self, lane0, lane1, skip, skip_idx):
        if isinstance(skip, tuple):
            skip0, skip1 = skip
        else:
            skip0 = skip1 = skip
    ...
    def _parallel_block(self, block_idx, lane0, lane1, x0, q_w, k_w, v_w, out_w, up_w, down_w):
    ...
    def forward_logits(self, input_ids):
        ...
        if self.embed_proj is not None:
            x = self.embed_proj(x)
        return self._forward_logits_from_embeddings(x)
...
@torch.compile
def _muon_fused_transform(g, momentum_buf, momentum, nesterov, row_normalize, backend_steps):
    new_buf = momentum_buf * momentum + g
    if nesterov:
        g = g + momentum * new_buf
    else:
        g = new_buf.clone()
    if row_normalize:
    ...
    if transposed:
        X = X.mT
    X = X / (X.norm(dim=(-2, -1), keepdim=True) + 1e-07)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for idx, m in enumerate(self._bank_meta):
                ...
                update, new_buf = _muon_fused_transform(g, buf, momentum, nesterov, row_normalize, backend_steps)
                buf.copy_(new_buf)
                if sharded:
        ...
        return loss
CONTROL_TENSOR_NAME_PATTERNS = tuple((pattern for pattern in os.environ.get('CONTROL_TENSOR_NAME_PATTERNS', 'attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gates,parallel_post_lambdas,parallel_resid_lambdas').split(',') if pattern))
...
def log_parallel_residual_converged(log0, model):
    ...
    log0(f'parallel_residual:converged active=1 start_layer={model.parallel_start_layer} start_mode={mode} final_lane={model.parallel_final_lane} freeze_lane0={int(model.parallel_freeze_lane0)} identity_init={int(model.parallel_identity_init)} skip_lane0_only={int(model.parallel_skip_lane0_only)} mlp_read_mix={int(model.parallel_mlp_read_mix)} used_layers={len(used_layers)}')
...
def _unbank_state_dict(state_dict, num_layers, attn_bank_slot=None, num_attn_layers=None):
    sd = {}
    n = num_layers
    if attn_bank_slot is None:
        attn_bank_slot = list(range(n))
        num_attn_layers = n
    na = max(num_attn_layers, 1)
    slot_to_layer = {}
    for layer_i, slot in enumerate(attn_bank_slot):
        if slot >= 0:
            slot_to_layer[slot] = layer_i
    for k, v in state_dict.items():
        t = v.detach().cpu()
        if k == 'qo_bank':
            for slot, layer_i in slot_to_layer.items():
                sd[f'blocks.{layer_i}.attn.c_q.weight'] = t[slot]
                sd[f'blocks.{layer_i}.attn.proj.weight'] = t[na + slot]
        elif k == 'kv_bank':
            for slot, layer_i in slot_to_layer.items():
                sd[f'blocks.{layer_i}.attn.c_k.weight'] = t[slot]
                sd[f'blocks.{layer_i}.attn.c_v.weight'] = t[na + slot]
        elif k == 'mlp_up_bank':
            for i in range(n):
...
def _rebank_state_dict(flat_sd, num_layers, model_dim, kv_dim, hidden_dim, attn_bank_slot=None, num_attn_layers=None):
    sd = {}
    n = num_layers
    if attn_bank_slot is None:
        attn_bank_slot = list(range(n))
        num_attn_layers = n
    na = max(num_attn_layers, 1)
    sd['qo_bank'] = torch.zeros(2 * na, model_dim, model_dim)
    sd['kv_bank'] = torch.zeros(2 * na, kv_dim, model_dim)
    sd['mlp_up_bank'] = torch.zeros(n, hidden_dim, model_dim)
    sd['mlp_down_bank'] = torch.zeros(n, model_dim, hidden_dim)
    for layer_i, slot in enumerate(attn_bank_slot):
        if slot < 0:
            continue
        sd['qo_bank'][slot] = flat_sd[f'blocks.{layer_i}.attn.c_q.weight']
        sd['qo_bank'][na + slot] = flat_sd[f'blocks.{layer_i}.attn.proj.weight']
        sd['kv_bank'][slot] = flat_sd[f'blocks.{layer_i}.attn.c_k.weight']
        sd['kv_bank'][na + slot] = flat_sd[f'blocks.{layer_i}.attn.c_v.weight']
    for i in range(n):
    ...
    for k, v in flat_sd.items():
        if not (k.startswith('blocks.') and any((p in k for p in ['.attn.c_q.', '.attn.c_k.', '.attn.c_v.', '.attn.proj.', '.mlp.fc.', '.mlp.proj.']))):
            sd[k] = v
    return sd
...
def collect_hessians(model, train_loader, h, device, n_calibration_batches=64):
    ...
    def make_mlp_hook(layer_idx):
        ...
        def hook_fn(module, inp, out):
            ...
            if h_act is not None:
                ...
                if name not in hessians:
                    hessians[name] = torch.zeros(h_act.shape[1], h_act.shape[1], dtype=torch.float32, device=device)
    ...
    for i, block in enumerate(model.blocks):
        if isinstance(block, Mamba2Block):
            hooks.append(block.attn.mamba.in_proj.register_forward_hook(
...
def serialize(h, base_model, code):
    ...
    sd_cpu = _unbank_state_dict(base_model.state_dict(), h.num_layers, base_model.attn_bank_slot, base_model.num_attn_layers)
...
def deserialize(h, device):
    ...
    flat_template = _unbank_state_dict(eval_model.state_dict(), h.num_layers, eval_model.attn_bank_slot, eval_model.num_attn_layers)
    ...
    kv_dim = h.num_kv_heads * head_dim
    hidden_dim = int(h.mlp_mult * h.model_dim)
    deq_state = _rebank_state_dict(deq_flat, h.num_layers, h.model_dim, kv_dim, hidden_dim, eval_model.attn_bank_slot, eval_model.num_attn_layers)
...
def eval_val_sliding_ttt(h, base_model, rank, world_size, device, val_data, stride):
    ...
    hash_emb = None
    orig_forward_logits = None
    if h.hash_embed_enabled:
        hash_emb = nn.Embedding(h.hash_embed_size, h.model_dim).to(device)
        nn.init.zeros_(hash_emb.weight)
        orig_forward_logits = base_model.forward_logits

        def forward_logits_with_hash(input_ids):
            x = base_model.tok_emb(input_ids)
            x = F.rms_norm(x, (x.size(-1),))
            prev_ids = torch.zeros_like(input_ids)
            prev_ids[:, 1:] = input_ids[:, :-1]
            h_idx = (prev_ids * 2039 + input_ids) % h.hash_embed_size
            x = x + hash_emb(h_idx)
            if base_model.embed_proj is not None:
                x = base_model.embed_proj(x)
            return base_model._forward_logits_from_embeddings(x)
        base_model.forward_logits = forward_logits_with_hash
    log(f'ttt_sliding:start chunks={num_chunks} chunk_tokens={ttt_chunk} total_windows={len(window_starts)} stride={stride} ttt_lr={h.ttt_lr} ttt_epochs={h.ttt_epochs} freeze_blocks={h.ttt_freeze_blocks} optimizer={h.ttt_optimizer} hash_embed={h.hash_embed_enabled}')
    ...
    if hash_emb is not None:
        ttt_params.append(hash_emb.weight)
    total_unfrozen = sum((p.numel() for p in ttt_params))
    total_frozen = sum((p.numel() for p in base_model.parameters() if not p.requires_grad))
    log(f'ttt_sliding:params unfrozen={total_unfrozen} frozen={total_frozen}')
    if h.ttt_optimizer == 'adamw':
        optimizer = torch.optim.AdamW(ttt_params, lr=h.ttt_lr, weight_decay=h.ttt_adamw_wd, fused=False)
    else:
    ...
    val_loss = (loss_sum / token_count).item()
    val_bpb = val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    if orig_forward_logits is not None:
        base_model.forward_logits = orig_forward_logits
    for p in base_model.parameters():
        p.requires_grad_(True)
    base_model.eval()
    log(f'ttt_sliding:done val_loss={val_loss:.6f} val_bpb={val_bpb:.8f} elapsed={time.perf_counter() - t0:.1f}s')
    return (val_loss, val_bpb)
...
def train_model(h, device, val_data):
    ...
    while True:
        ...
        with torch.no_grad():
            ...
            torch._foreach_add_(ema_vals, live_vals, alpha=1.0 - ema_decay)
        step += 1
...
def train_and_eval(h, device):
    ...
    if h.ttt_enabled:
        ...
        timed_eval('legal_ttt_exact', eval_val_sliding_ttt, h, ttt_model, h.rank, h.world_size, device, val_data, stride=h.eval_stride)
        del ttt_model
