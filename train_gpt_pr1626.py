...
class Hyperparameters:
    ...
    window_attn_layers = os.environ.get("WINDOW_ATTN_LAYERS", "")
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, q_w, k_w, v_w, out_w, cu_seqlens=None, max_seqlen=0):
        ...
        if cu_seqlens is not None:
            ...
            y = flash_attn_3_func(q, k, v, causal=True, window_size=self.window)
        if self.use_xsa:
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        window_set = set(int(s) for s in h.window_attn_layers.split(",") if s.strip())
        if window_set:
            win = (h.window_size - 1, 0)
            for i in window_set:
                self.blocks[i].attn.window = win
    ...
    def _block_with_lora(self, block, x, x0, lora, slot, q_w, k_w, v_w, out_w, up_w, down_w):
        ...
        y = flash_attn_3_func(q, k, v, causal=True, window_size=attn.window)
        if attn.use_xsa:
